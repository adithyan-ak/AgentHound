...
- `--insecure` is an explicit opt-out from certificate verification for authorized self-signed deployments. It applies to the direct poison, campaign round-trip, and `revert --insecure`; strict verification remains the default and automatic collection provenance still applies.
