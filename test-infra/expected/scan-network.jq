...
and ([.graph.nodes[] | select(.kinds | index("AIService"))] | length) == 8
...
and ([.graph.nodes[] | select(.kinds | index("LiteLLMGateway")) | .properties.endpoint])
  == ["http://10.20.30.15:4000"]
...
and ([.graph.nodes[] | select(
  (.kinds | index("AIService")) and
  (.properties.endpoint | contains("10.20.30.18"))
)] | length) == 0
