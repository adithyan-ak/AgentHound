...
- Unintended data exposure outside the documented single-operator trust boundary
...
- **Credential capture:** The collector intentionally stores exact observed credential values in its plain scan artifact and retains `value_hash` for identity/deduplication. Treat artifacts and the single-user server as operator-controlled offensive data; the dashboard masks Credential `value` until reveal/copy.
