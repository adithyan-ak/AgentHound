// Package litellmcollect implements the LiteLLM Collector. It turns a discovered
...
// Collect probes a LiteLLM gateway with the operator-supplied master key and emits
...
// Collector returns useful results and the failure list rather than
...
func (l *Collector) Collect(ctx context.Context, t action.Target, opts action.CollectOptions) (*action.CollectResult, error) {
	...
	if strings.TrimSpace(masterKey) == "" {
		return nil, errors.New("litellm collection requires compatible master-key material")
	}
	...
	// envelope is independently valid ingest v1. The collector knows the canonical
	...
	// 1. Build the master-key Credential candidate. It is removed below unless
	//    at least one authenticated LiteLLM endpoint accepts the material.
	...
	res.IngestData.Graph.Edges = append(res.IngestData.Graph.Edges,
		ingest.ExposesCredentialEdge(gatewayObjectID, masterID, "master_key", baseURL))
	...
	if modelInfoErr != nil {
		slog.Warn("litellm collection: /model/info failed",
	...
	if keyListErr != nil {
		slog.Warn("litellm collection: /key/list failed",
			"endpoint", keyListURL,
			"key_prefix", common.Redact(masterKey),
			"error", keyListErr)
	...
	// The candidate is already present in the scan graph. Attribute it to this
	// gateway only after at least one authenticated LiteLLM endpoint accepts it;
	// otherwise a rejected guess would become false EXPOSES_CREDENTIAL evidence.
	if modelInfoErr != nil && keyListErr != nil {
		res.IngestData.Graph.Nodes = removeGraphNode(res.IngestData.Graph.Nodes, masterID)
		res.IngestData.Graph.Edges = removeGraphEdgesForNode(res.IngestData.Graph.Edges, masterID)
		res.Summary.CredentialsFound--
	}

	slog.Info("litellm collection complete",
		"endpoint", baseURL,
		"key_prefix", common.Redact(masterKey),
...
func removeGraphNode(nodes []ingest.Node, id string) []ingest.Node {
	out := nodes[:0]
...
func removeGraphEdgesForNode(edges []ingest.Edge, id string) []ingest.Edge {
	out := edges[:0]
