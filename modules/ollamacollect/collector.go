// Package ollamacollect implements the Ollama Collector.
//
// Ollama commonly exposes anonymous model inventory and modelfiles through
// read-only HTTP lookups. The Collector emits one
...
//
// Probes (GET-only by default — Collectors are read-only by contract):
//
...
//
//	POST /api/embeddings      (deep active mode)
//	    Issues a single benchmark embedding request to confirm the
//	    inference compute path is consumable. The Collector contract is
...
// ~/.ollama/models/blobs/, which is out of scope for a Collector.
...
// Collect probes an Ollama instance, emits one :OllamaInstance node, one
// :AIModel per /api/tags entry with PROVIDES_MODEL edges, and (when
// deep active mode) issues an embedding probe.
//
...
func (l *Collector) Collect(ctx context.Context, t action.Target, opts action.CollectOptions) (*action.CollectResult, error) {
	...
	if err != nil {
		slog.Warn("ollama collection: /api/tags failed",
			"endpoint", tagsURL,
			"error", err)
	...
	for _, tag := range tags {
		...
		res.Summary.EndpointsProbed++
		if showErr != nil {
			slog.Warn("ollama collection: /api/show failed",
				"endpoint", showURL,
				"model", tag.Model,
				"error", showErr)
	...
	slog.Info("ollama collection complete",
		"endpoint", baseURL,
...
func providesModelEdge(ollamaID, modelID, digest string) ingest.Edge {
	return ingest.Edge{
		Source:     ollamaID,
		Target:     modelID,
		Kind:       "PROVIDES_MODEL",
