...
func TestCollect_JupyterHappy(t *testing.T) {
	srv := jupyterStub(t)
	defer srv.Close()
...
func TestCollect_Jupyter_SessionsFail(t *testing.T) {
...
func TestCollectJupyterRejectsMalformedSuccessfulContents(t *testing.T) {
...
// TestCollect_Jupyter_RecursiveContents — a 3-level tree
// (/api/contents/, /api/contents/a, /api/contents/a/b) must be walked
// entirely and all leaves emitted.
func TestCollect_Jupyter_RecursiveContents(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Path {
		case "/api/sessions":
...
// TestCollect_Jupyter_MaxDepthCap — with --max-depth=2, notebooks at
// depth 3 (a/b/deep.ipynb) must not be emitted.
func TestCollect_Jupyter_MaxDepthCap(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Path {
		case "/api/sessions":
...
// TestCollect_Jupyter_SubdirForbidden — root lists 3 subdirs; one returns
// 403; the walker records a per-directory partial failure and emits
// notebooks from the other two subdirs.
func TestCollect_Jupyter_SubdirForbidden(t *testing.T) {
	var (
		mu   sync.Mutex
		hits = map[string]int{}
	)
...
func TestCollect_RetriesDeniedRootsAndPropagatesSuppliedBearer(t *testing.T) {
...
func TestCollect_DoesNotSendSuppliedTokenWhenAnonymousControlsSucceed(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestCollect_WrongTokenDoesNotInventBearerAuthentication(t *testing.T) {
	var (
		mu   sync.Mutex
		hits = map[string][]string{}
	)
...
func TestCollect_MixedProtectedAuthorizationRemainsUnknown(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Path {
		case "/api/sessions":
...
func TestCollect_PasswordOrCustomDenialDoesNotGuessMechanism(t *testing.T) {
	var requests int
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requests++
		if got := r.Header.Get("Authorization"); got != "" {
...
func TestCollect_RejectsMalformedTokenBeforeNetworkAccess(t *testing.T) {
	tests := []string{
		"Bearer",
		"Basic secret-token",
		"Bearer one two",
	...
	for _, token := range tests {
		t.Run(strings.ReplaceAll(token, " ", "_"), func(t *testing.T) {
			...
			_, err := (&Collector{}).Collect(context.Background(), action.Target{
				Address: strings.TrimPrefix(srv.URL, "http://"),
			}, action.CollectOptions{
				Credentials: map[string]string{"token": token},
			})
			if err == nil {
				t.Fatal("Collect error = nil, want malformed token rejection")
			}
