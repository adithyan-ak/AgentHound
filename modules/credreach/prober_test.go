...
func TestClassifyStructuredErrors(t *testing.T) {
	...
	if got := classifyProbeError(context.Background(), headerMismatch); got != ProbeMalformedAuth {
		t.Fatalf("header-mismatch code => %q, want malformed_auth", got)
	}
	authWire := &jsonrpc.Error{Code: -32000, Message: "Unauthorized"}
	if got := classifyProbeError(context.Background(), authWire); got != ProbeAmbiguous {
...
func TestClassifyErrorMessages(t *testing.T) {
	cases := []struct {
		...
		{"http 401 Unauthorized", ProbeAmbiguous},
		{"received 403 Forbidden", ProbeAmbiguous},
		{"permission denied by policy", ProbeAmbiguous},
		{"access denied: credential rejected", ProbeAmbiguous},
		{"server returned 404 Not Found", ProbeNotFound},
		{"resource not found on server", ProbeNotFound},
		{"400 Bad Request: malformed header", ProbeMalformedAuth},
		{"connection refused", ProbeError},
		{"x509: certificate signed by unknown authority", ProbeError},
		{"json-rpc parse error", ProbeProtocolError},
		{"something totally unexpected", ProbeAmbiguous},
	}
...
func TestOnlyTypedObservedHTTPAuthStatusIsDenied(t *testing.T) {
	for _, status := range []int{http.StatusUnauthorized, http.StatusForbidden} {
		...
		if got := classifyProbeError(context.Background(), err); got != ProbeDenied {
	...
	for _, wire := range []*jsonrpc.Error{
		...
		if got := classifyProbeError(context.Background(), wire); got != ProbeAmbiguous {
...
func TestObservedHTTPAuthResponseIsDenied(t *testing.T) {
	for _, status := range []int{http.StatusUnauthorized, http.StatusForbidden} {
		t.Run(http.StatusText(status), func(t *testing.T) {
			...
			result := (&mcpProber{}).Probe(context.Background(), ProbeRequest{
				Host: srv.URL, ResourceURI: "test://resource", Timeout: time.Second,
			})
			if result.Stage != ProbeStageInitialize || result.Status != ProbeDenied {
...
func TestClassifyTimeout(t *testing.T) {
	...
	cancel()
	if got := classifyProbeError(context.Background(), context.DeadlineExceeded); got != ProbeTimeout {
...
func TestNotFoundStaysIndeterminate(t *testing.T) {
	...
	if got := Classify(control, authed); got != OutcomeIndeterminate {
...
func TestMCPProbeHangingCloseUsesOriginalDeadlineAndIsCounted(t *testing.T) {
	...
	started := time.Now()
	go func() {
		resultCh <- (&mcpProber{}).Probe(runCtx, ProbeRequest{
...
func TestProbeHeaderRoundTripperExactOrigin(t *testing.T) {
	origin, err := common.ParseHTTPOrigin("https://Example.COM/mcp")
	if err != nil {
		t.Fatal(err)
	}
	cases := []struct {
