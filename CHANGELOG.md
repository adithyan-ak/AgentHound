...
Prelaunch **truthfulness contract reset**: scans, graph facts, findings, and every UI verdict are now explicitly scoped, completeness-aware, and evidence-backed. Generation-scoped ingest only promotes a fully materialized generation to the default read target; findings carry a typed evidence DAG, nullable attack cost, and lifecycle/confidence metadata; APIs disclose scope, totals, pagination, and completeness; the disconnected global Inspector and the unproduced `ResourceGroup` / `TrustZone` kinds are removed; and the frontend graph/auth registry is generated from the Go source of truth. Because there are no users yet, the artifact/API contracts changed in place and a clean database reset + re-ingest is required.

### Truthful evidence, disclosure, and identity enforcement

- **Honest attack-path rendering.** The finding-detail "Attack Path" strip no longer fabricates a `—` edge between source and target when no path was reconstructed, and no longer silently linearizes branched/disconnected evidence into one invented chain. It renders the evidence in observed edge order, marks genuine discontinuities with an explicit non-edge **gap** (never a proven hop), discloses when the evidence is not a single continuous chain, and shows the real hop count (or "path unresolved"). Attack cost already renders `Unknown` — never a fabricated `0.0` — when any edge weight is missing. New pure helper `buildPathSpine` with unit coverage.
- **Blast-radius direction + hop-limit disclosure.** The Explorer blast-radius lens now exposes and discloses its traversal: an inbound / outbound / both direction control and a bounded hop-limit stepper (1–10), with a plain-language line stating exactly which directional, bounded slice the rings represent (previously fixed at outbound / 6 hops with no disclosure).
- **Scope-qualified centrality and isolation copy.** Chokepoint ranking is labeled as computed across *all loaded edges* (with a partial-read caveat when the graph is truncated), and "unconnected" nodes are qualified as lens-relative — a node with no edges in the current lens may still connect under another lens or beyond a truncated read — rather than implying global isolation.
- **Server-side identity-version enforcement.** The ingest validator rejects any artifact whose `meta.identity_version` is present and unsupported (the node-identity derivation scheme is versioned; a mismatch would compute non-matching IDs and silently fail to merge). A zero/absent value defers to the current scheme. Documented in `docs/reference/api.md` and `docs/reference/graph-model.md` (the stale `sorted_args` MCPServer-ID description was corrected to the order-preserving identity-v2 scheme).
- **Generated-registry staleness gate.** A Go test now fails if `server/ui/src/entities/graph/generated.ts` drifts from the canonical Go graph registry, so the TypeScript node/edge/enum unions can never fall behind `sdk/ingest` + `sdk/common` (regenerate with `go run ./server/cmd/gengraphts`).

### Final integrated review fixes (multi-scope truthfulness)

- **Loot scope + restored cross-service credential chain.** A `loot` artifact now occupies its own generation scope (`loot:<type>`) instead of colliding with the local `scan:local` bundle, so it never demotes a config/mcp/a2a bundle. Because the Config Collector and a Looter ship as separate generations, the credential chain is re-run cross-generation (`analysis.CrossGenerationCredentialChain`) over the explicitly selected current collector generations plus the one being ingested, joining `Credential.value_hash` across them and emitting the dedicated `CAN_REACH_CREDENTIAL_CHAIN` edge.
- **Deterministic logical-current projection.** Multi-scope API stats/list/detail/search/edge reads project physical observations to one logical node per `objectid` (union of properties, deterministic greatest-`generation_id` conflict winner). Stats count `DISTINCT objectid`/triples, so duplicate physical observations of the same `objectid` (the config+mcp `MCPServer` merge point) never leak while merging still works.
- **Durable, recoverable scan deletion.** A delete persists a `deleting`/`delete_failed` lifecycle before mutating the graph, removes the generation's contribution in a single Neo4j transaction, and exposes the rematerialized prior generation only after the recoverable delete completes. A failure is retryable idempotently, and a startup sweep resumes interrupted deletes (migration `007_scan_delete_lifecycle`).
- **Promotion durability + stage-aware completeness.** Successful prerequisite stage outcomes are persisted before the current pointer flips, and API completeness now requires every promoted generation's write/post-processing/snapshot stages to have succeeded — coverage-complete alone is no longer sufficient.
- **Generation-scoped analysis endpoints.** Shortest/all/weighted-path and pre-built analysis queries are scoped to the current logical generations (every path node/edge and query anchor must be in a promoted generation); each response carries a `completeness` disclosure. Weighted path reports `algorithm: bounded-min-weight`.
- **Edge inventory as distinct logical triples.** The per-generation edge inventory delta is computed as distinct `(source, kind, target)` triples diffed against the prior current generation, not physical relationship rows.
- **Correct Neo4j constraint syntax.** Schema init emits `ON ... ASSERT` on Neo4j 4.x (including 4.4) and `FOR ... REQUIRE` on 5.x+ (keyed strictly on the major version).
