...
- **A2A runtime authentication evidence:** after card parsing, the collector performs one deduplicated, credential-free, read-only nonexistent-task lookup against an authorized same-origin preferred JSON-RPC endpoint. v1 uses `GetTask` with `A2A-Version`; v0.3 uses `tasks/get`. Only an exact matching TaskNotFound response emits `none/unauthenticated/anonymous_probe_succeeded`; 401/403 records a protected diagnostic and every redirect, timeout, version/method error, malformed/HTML/oversized response, cross-origin interface, or userinfo/query/fragment-bearing endpoint remains unknown without an observed tuple. Requests are capped at five seconds and 64 KiB, random IDs never enter artifacts, and no message, cancellation, executor, or task mutation path is used. The real-infrastructure lane now proves exact official-SDK v1/v0.3 positives, an API-key denial, an ambiguous-version negative, zero credential forwarding, and zero executor/task-store mutation.
- **Looter authentication truth:** direct Ollama, MLflow, and Qdrant looting now emits a neutral service/attempt node until the first credential-free inventory operation succeeds with its canonical response shape (`models[]`, `experiments[]`, or `status=ok` plus `result.collections[]`). Only that success adds `none/unauthenticated/anonymous_probe_succeeded`, verified time, and the legacy anonymous-loot flag. HTTP denial, transport failure, malformed JSON, and 2xx wrong-shaped bodies cannot fabricate anonymous evidence or an empty inventory.
